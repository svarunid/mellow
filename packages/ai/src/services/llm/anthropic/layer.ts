	...
	export const make = (overrides?: Overrides) =>
		Layer.effect(
			...
			Effect.gen(function* () {
				...
				const envBaseUrl = yield* Config.string("ANTHROPIC_BASE_URL").pipe(
					Config.withDefault("https://api.anthropic.com"),
				);
				...
				const buildParams = (params: GenerateTextParams, stream: boolean) => {
					...
					if (params.maxTokens !== undefined) base.maxTokens = params.maxTokens;
					...
					if (params.topK !== undefined) base.topK = params.topK;
				...
				const generateText = (
					...
					Effect.gen(function* () {
						...
						const parsed = yield* Effect.try({
							try: () => Schema.decodeUnknownSync(AnthropicMessagesResponse)(raw),
							catch: (e) =>
